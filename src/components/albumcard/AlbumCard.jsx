...
const AlbumCard = (props) => {
    const { data: relatedData, error } = useGetRelatedQuery("")
    ...
    return (
        <div>
            <Grid container>
                <Box className="grid grid-cols-1 xs:grid-cols-2   sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-7 mx-auto mt-10  p-4 gap-8">
                    ...
                    {relatedData?.data.slice(0, 14).map((items) => (
                        <Box key={items.id} sx={{ width: 210, marginRight: 0.5, my: 5 }}>
                            <div className=" Card  text-white bg-smooth-black w-full">
                              ...
                              justify-between leading-tight p-2 md:p-4">
                                    {items ? (
                                        <Box sx={{ pr: 2 }}>
